...
**Not yet covered (CPU path stays):** estimate_entropy_full's full
cost formula (`info_loss_mul * info_loss + zeros_mul * nzeros`
re-weighting) — needs a kernel extension on
entropy_coeffs_pixel_blocks_gpu (currently fills only entropy_sum
+ nzeros_sum). **estimate_entropy_full DCT8 orchestrator landed as
of 2026-05-08** — `forks::cost::estimate_entropy_full_dct8_batch_gpu`
composes ~12 GPU launches per call (3 forward DCT + 3 entropy + 3
IDCT + 3 pixel-loss) plus host-side combiners
(`extract_per_block_entropy`, `sum_per_block_entropy_3channel`,
`combine_pixel_loss_3channel`, `per_block_total_cost`) into a
per-block cost evaluator for the AC strategy search. **Mixed-strategy
