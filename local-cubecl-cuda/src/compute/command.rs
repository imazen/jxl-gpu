...
impl<'a> Command<'a> {
    ...
    pub fn reserve_cpu(
        ...
        // FORK PATCH (jxl-encoder-gpu, 2026-05-13): bumped pinned cap
        // from 100 MB to 1 GB. Our 144 MB xyb plane downloads were
        // falling back to pageable bounce-buffer; raw cudarc bench
        // shows pinned is 5-10× faster at this size.
        const FORK_PINNED_CAP_BYTES: usize = 1024 * MB;
        if !marked_pinned && size > FORK_PINNED_CAP_BYTES {
            return Bytes::from_bytes_vec(vec![0; size]);
        }

        match self.reserve_pinned(size, origin) {
            Some(b) => b,
            None => {
                if std::env::var_os("JXL_GPU_TRACE_PINNED_FALLBACK").is_some() {
                    eprintln!(
                        "cubecl reserve_pinned FAILED for {size} bytes \
                         (marked_pinned={marked_pinned}); falling back to pageable"
                    );
                }
                Bytes::from_bytes_vec(vec![0; size])
            }
        }
    }
