...
use crate::kernels::entropy::{
    entropy_coeffs_coeff_kernel, entropy_coeffs_coeff_kernel_broadcast_w,
    entropy_coeffs_pixel_kernel, entropy_coeffs_pixel_kernel_broadcast_w,
};
...
/// Broadcast-weights launcher for
/// [`entropy_coeffs_coeff_kernel_broadcast_w`]. `inv_weights` is
/// exactly `n` f32 (one inverse-quant matrix), broadcast across all
/// blocks.
#[allow(clippy::too_many_arguments)]
pub fn entropy_coeffs_coeff_broadcast_w<R: Runtime>(
    client: &ComputeClient<R>,
    block_c: Handle,
    block_y: Handle,
    inv_weights: Handle, // n f32
    ...
    let cubes = num_blocks.max(1);
    unsafe {
        entropy_coeffs_coeff_kernel_broadcast_w::launch_unchecked::<R>(
            client,
            CubeCount::Static(cubes, 1, 1),
            CubeDim::new_1d(1),
            ArrayArg::from_raw_parts(block_c, n_total),
