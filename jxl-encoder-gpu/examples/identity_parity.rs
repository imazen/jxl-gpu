//! Parity test: GPU IDENTITY transform vs
//! `jxl_encoder::vardct::dct::{identity_transform, inverse_identity_transform}`.
//!
//! ```sh
//! cargo run --release --example identity_parity --features cuda
//! ```
...
fn main() {
    use cubecl::prelude::*;
    use jxl_encoder::vardct::dct::{identity_transform, inverse_identity_transform};
    use jxl_encoder_gpu::launch::identity::{identity_forward, identity_inverse};
    ...
    // Deterministic input: small magnitude values per block.
    ...
    let ok_fwd = m < 1e-6;
    println!(
        "identity_forward (n={N_BLOCKS}):  max|Δ| = {m:.3e} at idx {p}  {}",
        if ok_fwd { "✓" } else { "✗" }
    );
    if !ok_fwd {
        eprintln!("  cpu[{p}]={:.6}  gpu[{p}]={:.6}", cpu_fwd[p], gpu_fwd[p]);
    }

    // CPU inverse pass on the CPU forward result (for roundtrip parity).
    ...
    let ok_inv = m2 < 1e-6;
    println!(
        "identity_inverse (n={N_BLOCKS}):  max|Δ| = {m2:.3e} at idx {p2}  {}",
        if ok_inv { "✓" } else { "✗" }
    );
    if !ok_inv {
        eprintln!(
    ...
    identity_inverse::<Backend>(&client, h_round_in, h_round_out.clone(), N_BLOCKS as u32);
    ...
    println!(
        "identity roundtrip (n={N_BLOCKS}):  max|Δ| vs original = {m3:.3e} at idx {p3}  {}",
        if ok_round { "✓" } else { "✗" }
    );
